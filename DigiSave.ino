...
enum Screen { WIFI_SCAN, PASSWORD_INPUT, CONNECTING, HOME };
...
void drawSingleNetwork(int index) {
  ...
  if (displaySSID.length() > 18) { // Limit characters for fixed size
...
void handleKeyboardInput() {
  ...
  if (millis() - lastCursorUpdate > 50) {  // Update every 50ms for smooth cursor blinking
    updateCursor();
